...
// PostgresRepository 负责将提醒任务持久化到 PostgreSQL。
type PostgresRepository struct {
	db *sql.DB
}

// NewPostgresRepository 创建一个基于共享数据库连接池的提醒仓储。
...
func (r *PostgresRepository) ListDueTasks(ctx context.Context, now time.Time, limit int) ([]Task, error) {
	const query = `
SELECT
...
func (r *PostgresRepository) MarkDone(ctx context.Context, id int64, lastRun time.Time) error {
	const query = `
...
func (r *PostgresRepository) RescheduleDaily(ctx context.Context, id int64, lastRun, nextRun time.Time) error {
	const query = `
UPDATE reminder_tasks
